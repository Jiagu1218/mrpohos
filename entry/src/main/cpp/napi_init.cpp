...
/* RequestBuffer 常给未初始化的新缓冲；只写 dirty 矩形其余为垃圾会闪屏。在此保留与 MRP 对齐的整屏 RGB565 再整幅上传。 */
static uint16_t g_mrpRgb565Composite[(size_t)SCREEN_WIDTH * (size_t)SCREEN_HEIGHT];
...
/** RequestBuffer → RGB565 直通拷贝 → Flush。已持 g_windowMutex 且 g_nativeWindow 非空。 */
